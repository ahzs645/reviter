...
test("replaces source guide lines with topology outlines from visible meshes", () => {
  ...
  assert.equal(
    (batched.userData.batchStats as AutodeskSceneBatchStats).sourceLineSegments,
    2,
  );
  assert.equal(
    (batched.userData.batchStats as AutodeskSceneBatchStats).lineBatches,
    1,
  );
});
