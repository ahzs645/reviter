/**
 * The recovered-model `ConvertResult` the IFC export tests are written against.
 *
 * Extracted from `export-ifc.test.ts` so a second reader can be pointed at the
 * same export without the two tests drifting apart: a fixture copied into the
 * independent-reader test would let one of them start proving something about
 * an export the other no longer produces.
 */
import type { ConvertResult } from "../../lib/reviter/types.ts";

export function ifcExportFixture(): ConvertResult {
  return {
    ok: true,
