...
export type ReferenceElementFragment = {
  object: THREE.BatchedMesh;
  batchId: number;
};

export type ReferenceSceneBatchStats = {
...
export type ReferenceOutlineMode = "orbit" | "walk";

export function setReferenceOutlineMode(
  root: THREE.Object3D,
  mode: ReferenceOutlineMode,
): void {
  root.traverse((object) => {
    if (object.userData.source === "reference-runtime-batch"
...
function referenceOutlineMaterial(): THREE.LineBasicMaterial {
  ...
  material.name = "Reviter reference depth-aware outlines";
...
function createReferenceOutlines(
  ...
  const lines = new THREE.LineSegments(geometry, referenceOutlineMaterial());
  lines.name = "Autodesk generated topology outlines";
  lines.castShadow = false;
  lines.receiveShadow = false;
  lines.renderOrder = 4;
  lines.userData = {
    source: "reference-runtime-batch",
    primitive: "lines",
 ...
 */
export function batchReferenceScene(root: THREE.Object3D): THREE.Group {
  ...
  const elementFragments = new Map<string, ReferenceElementFragment[]>();
  ...
  const outlines = createReferenceOutlines(meshEntries);
  if (outlines) output.add(outlines);

  const stats: ReferenceSceneBatchStats = {
  ...
  output.userData = {
    ...root.userData,
    source: root.userData.source ?? "paired-reference-model",
    runtimeBatched: true,
    batchStats: stats,
    walkSurface,
    elementFragments,
