...
| Certified browser geometry | 16,254 complete owners retained; 35,198 scene elements and 823,452 native triangles emitted; 1,350 elements retain proxy/other geometry | Current TypeScript replay supplies a bounded subset of the graph the native builder/renderer would consume |
| Family regeneration | 7,714 / 7,805 production placement owners resolve without regeneration; 91 remain absent, conflicting, or incomplete | `GeomTable` retains generator-ID indirection, not a drawable graph; the dynamic `GeomStepList`/history state or an actual `m_geometry` BRep must be reconstructed first, and none of the named BRep entry points accepts a `FamilySymbol` record |
| General BRep/tessellation | Planes plus bounded cylinder/cone/surface-of-revolution paths, three exact non-legacy shapes, the schema-complete conditioned-Geometry route, and 169 transformed embedded-GElement columns are certified; the original 151 candidates still yield 141 exact bounded roots | Builder/traversal/renderer exports define the needed neutral contract, but arbitrary cylinder trims and general NURBS/Hermite refinement still depend on policies not yet proven from the RVT |
