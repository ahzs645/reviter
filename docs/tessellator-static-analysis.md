...
| Certified browser geometry | 16,085 complete owners retained; 35,029 scene elements and 810,748 native triangles emitted; 1,519 elements retain proxy/other geometry | Current TypeScript replay supplies a bounded subset of the graph the native builder/renderer would consume |
...
| General BRep/tessellation | Planes plus bounded cylinder/cone/surface-of-revolution paths, three exact non-legacy shapes, and the schema-complete `GFilter`/`GPoint`/`GConditionInt` conditioned-Geometry route are certified; the original 151 candidates still yield 141 exact bounded roots | Builder/traversal/renderer exports define the needed neutral contract, but general NURBS/Hermite refinement still depends on runtime view/triangulation inputs not proven from the RVT |
