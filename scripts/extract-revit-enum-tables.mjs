...
/**
 * Extract the embedded label resource from a third-party interoperability SDK.
 *
 ...
 * independent source. No third-party code, algorithm, or binary is reproduced.
 *
 * Usage:
 *   node scripts/extract-revit-enum-tables.mjs [isolated-root] [--write-lib]
 */
...
const markdown = [
  ...
  "",
  `Extracted from \`${BINARY}\` by \`scripts/extract-revit-enum-tables.mjs\`.`,
 ...
 *
 * The reader composes a class from the file's own schema, so these are not layouts.
...
if (!writeLib) {
  console.log("\npass --write-lib to regenerate lib/reviter/revit-enum-tables.ts");
  process.exit(0);
}
